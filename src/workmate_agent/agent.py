...
# Cognitive Services scope used for the app-identity bearer token sent to Azure
# OpenAI when no signed-in user is available (fallback path).
AOAI_SCOPE = "https://cognitiveservices.azure.com/.default"

# Scope for the agentic-user (OBO) token forwarded as the TOP-LEVEL Authorization
# bearer on the Responses call. Foundry resolves user-delegated IQ connections
# (Work IQ = UserEntraToken, Fabric Data Agent) on behalf of this user. Matches
# refund-agent-a365's FOUNDRY_USER_TOKEN_SCOPES.
FOUNDRY_USER_SCOPE = "https://ai.azure.com/.default"

# Responses API version. 2025-11-15-preview is what the live refund-processor
# (refund-agent-a365) uses against this same project.
AOAI_API_VERSION = "2025-11-15-preview"

# Full ARM connection id of the Fabric Data Agent connection in 4iq-foundry-project.
_FABRIC_CONNECTION_ID = (
    "/subscriptions/27b0139a-16b4-42bf-9ec9-c6db3768245e/resourceGroups/rg-aycabas-3iqs"
    "/providers/Microsoft.CognitiveServices/accounts/4iq-foundry-project-resource"
    "/projects/4iq-foundry-project/connections/fabric_dataagent_preview_b75c99"
)
...
class FoundryDigitalWorkerAgent(AgentInterface):
    ...
    def __init__(self) -> None:
        ...
        # Workmate persona instructions (Work IQ) loaded from instructions.md.
        self._instructions = self._load_instructions()

        # Fabric Data Agent connection id (project connection in 4iq-foundry-project).
        self._fabric_connection_id = (
            os.getenv("FABRIC_CONNECTION_ID") or _FABRIC_CONNECTION_ID
        )
    ...
    def _load_instructions(self) -> str:
        ...
        try:
            ...
            if text:
                logger.info("Loaded instructions from %s (%d chars)", path, len(text))
                return text
        except OSError:
            logger.warning("instructions.md not found at %s; using default prompt", path)
    ...
    async def process_user_message(
        ...
        personalized_prompt = self._instructions.replace("{user_name}", display_name)
...
""".strip()
    ...
    async def _invoke_responses_api(
        ...
        *,
        ...
        iq_tools = self._build_iq_tools()
        logger.info(
            "Invoking Responses API with %d IQ tool(s)", len(iq_tools)
        )
        ...
        # Prefer the agentic-user (OBO) token so Foundry resolves user-delegated IQ
        # connections (Work IQ, Fabric Data Agent) on behalf of the signed-in user.
        # Fall back to app identity (API key / managed identity) when no user token
        # is available (e.g. local smoke tests) — only Web IQ + Foundry IQ resolve then.
        user_token = await self._acquire_foundry_user_token(
    ...
    def _build_iq_tools(self) -> list[dict[str, Any]]:
        """Build the Work IQ tool as a raw Foundry Responses tool definition.

        This is a Work IQ Deep Dive agent, so it exposes **only** the Work IQ
        connection (in 4iq-foundry-project) by ``project_connection_id``; Foundry
        resolves it server-side. The Work IQ connection is ``UserEntraToken`` (OBO),
        so it is resolved on behalf of the signed-in user via the agentic-user
        bearer set on the Responses call.

        ``ask`` is the retrieval entry point; ``search_paths``/``get_schema``/
        ``fetch`` do grounded reads; ``do_action`` is how Work IQ performs actions
        (send mail, post a Teams message, etc.) — the MCP-only write path.
        """
        work_iq = {
            "type": "mcp",
            "server_label": "WorkIQ",
            "server_url": os.getenv("WORK_IQ_MCP_URL", "https://workiq.svc.cloud.microsoft/mcp"),
            "project_connection_id": os.getenv("WORK_IQ_CONNECTION_ID", "WorkIQ"),
            "allowed_tools": [
                "ask",
                "search_paths",
                "get_schema",
    ...
    async def _acquire_foundry_user_token(
        self,
        auth: Authorization,
        auth_handler_name: Optional[str],
        context: TurnContext,
    ) -> Optional[str]:
        """Exchange for the agentic-user (OBO) token used as the top-level Responses
        bearer, so Foundry resolves user-delegated IQ connections as the signed-in
        user. Mirrors refund-agent-a365's _exchange_user_token (scope ai.azure.com).
        """
        ...
        try:
            ...
            if not token:
                logger.warning("Agentic-user token exchange returned no token")
            return token
        except Exception:
            logger.exception("Failed to acquire agentic-user token for Foundry")
            return None
