...
class GenericAgentHost:
    ...
    def __init__(
        ...
        **agent_kwargs,
        ...
        # Idempotency guard: Azure Bot Service / the Foundry activity endpoint
        # re-delivers a turn when the bot doesn't return fast enough (~10s). Refund
        # answers hit the LLM + IQs and can exceed that, so the same answer was sent
        # to Teams twice. The redelivery arrives with a DIFFERENT activity id, so we
        # dedup on (conversation id + message text) within a short window, and also
        # on the raw activity id as a cheap first check.
        self._processed_activity_ids: "OrderedDict[str, float]" = OrderedDict()
        self._processed_content_keys: "OrderedDict[str, float]" = OrderedDict()
        self._dedup_lock = asyncio.Lock()
        self._dedup_ttl_seconds = 900
        self._content_dedup_ttl_seconds = 120
        self._dedup_max_ids = 2048
    ...
    async def _seen_activity_before(self, activity_id: Optional[str]) -> bool:
        """Return True if this inbound activity id was already processed.

        Used to drop Azure Bot Service delivery retries (which reuse the same
        activity id) so a slow turn doesn't produce duplicate replies. Marks
        the id as seen atomically and prunes old / excess entries.
        """
        if not activity_id:
            return False
        now = time.monotonic()
        async with self._dedup_lock:
            self._prune_locked(self._processed_activity_ids, now, self._dedup_ttl_seconds)
            if activity_id in self._processed_activity_ids:
                return True
            self._processed_activity_ids[activity_id] = now
            while len(self._processed_activity_ids) > self._dedup_max_ids:
                self._processed_activity_ids.popitem(last=False)
            return False

    async def _seen_content_before(self, conversation_id: str, text: str) -> bool:
        """Return True if the same message text for the same conversation was
        processed within the content-dedup window.

        The Teams/Bot Service/Foundry-endpoint chain re-delivers turns that take
        longer than ~10s, and the redelivery carries a *different* activity id,
        so activity-id dedup misses it. Keying on (conversation, text) within a
        short window catches those redeliveries. A user genuinely repeating the
        exact same text within the window is treated as a duplicate (acceptable
        for this agent).
        """
        key = f"{conversation_id}|{text.strip()}"
        now = time.monotonic()
        async with self._dedup_lock:
            self._prune_locked(
                self._processed_content_keys, now, self._content_dedup_ttl_seconds
            )
            if key in self._processed_content_keys:
                return True
            self._processed_content_keys[key] = now
            while len(self._processed_content_keys) > self._dedup_max_ids:
                self._processed_content_keys.popitem(last=False)
            return False

    @staticmethod
    def _prune_locked(store: "OrderedDict[str, float]", now: float, ttl: float) -> None:
        cutoff = now - ttl
        while store:
            _, ts = next(iter(store.items()))
            if ts < cutoff:
        ...
        async def on_message(context: TurnContext, _: TurnState) -> None:
            try:
                ...
                with BaggageBuilder().tenant_id(tenant_id).agent_id(agent_id).build():
                    ...
                    if await self._seen_activity_before(context.activity.id):
                        logger.info(
                            "🔁 Duplicate delivery for activity %s — skipping "
                            "reprocessing (ack only)",
                            context.activity.id,
                        )
                        return
                    ...
                    if not is_email_activity(context.activity):
                        ...
                        if await self._seen_content_before(conversation_id, user_message):
                            logger.info(
                                "🔁 Duplicate re-delivery (same text, new activity id) "
                                "for conversation %s — skipping (ack only)",
                                conversation_id,
                            )
                            return
