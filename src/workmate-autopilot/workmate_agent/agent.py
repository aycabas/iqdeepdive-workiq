...
class FoundryDigitalWorkerAgent(AgentInterface):
    ...
    AGENT_PROMPT = (
        "You are Workmate, a Microsoft 365 digital-worker assistant grounded in the "
        "signed-in user's workplace context through Work IQ.\n"
        "Help the user achieve their objectives using their email, calendar, Teams "
        "chats, files, and people data — all reached through the Work IQ MCP tools.\n\n"
        ...
        "Do not overuse it.\n\n"
        "# Work IQ tools\n"
        "- Work IQ exposes MCP tools such as `ask` (natural-language questions over the "
        "user's work graph), `search_paths`/`get_schema`/`fetch` (typed reads of mail, "
        "calendar, chats, files, people) and `do_action` (take actions such as sending "
        "mail or Teams messages).\n"
        "- For most questions, prefer `ask` — it grounds the answer in the user's real "
        "workplace data.\n"
        "- To take an action (send an email, reply to a chat, create an event), use "
        "`do_action` with the correct action URL and payload.\n\n"
        "# General\n"
        "- Ground every answer about the user's mail, meetings, chats, or files in the "
        "Work IQ tools; do not invent facts.\n"
        "- Distinguish sourced facts from inference, and mention relevant dates and "
        "participants.\n"
        ...
        "- Use the Work IQ `do_action` tool to send email.\n"
        "- You can use the AAD object ID inside the Activity context's 'From' field to "
        "determine where to respond to emails from.\n\n"
        "For Teams messages, only send a Teams message when the user explicitly asks "
        "you to. Otherwise, do not send one.\n\n"
...
""".strip()
    ...
    async def _build_mcp_tools(
        ...
        for server in self._mcp_servers:
            ...
            if not url:
                continue
            scope = self._exchange_scope_for_server(server)
            if scope not in token_by_scope:
                token_by_scope[scope] = await self._acquire_mcp_token(
                    auth, auth_handler_name, context, scope
                )
            bearer = token_by_scope[scope]
            if not bearer:
                logger.warning(
                    "No MCP bearer token for server %s (scope=%s); "
    ...
    @staticmethod
    def _exchange_scope_for_server(server: dict[str, Any]) -> str:
        """Resolve the token-exchange scope for one manifest server entry.

        Uses the entry's ``audience`` (``{audience}/.default``) so each MCP
        server can request a token for its own resource — e.g. the A365 tool
        servers (``ea9ffc3e-...``) and Work IQ
        (``api://workiq.svc.cloud.microsoft``). Falls back to the legacy
        hardcoded A365 scope when no audience is present.
        """
    ...
    async def _acquire_mcp_token(
        ...
        try:
            ...
            logger.exception(
                "Failed to acquire MCP bearer token for scope %s", scope
            )
            return None
