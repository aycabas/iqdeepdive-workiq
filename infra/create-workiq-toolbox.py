"""Create the Entra app, OAuth2 RemoteA2A connection, and toolbox for the Work IQ agent.

Adapted from pamelafox/iqdeepdive-foundryiq/infra/create-workiq-toolbox.py for the Work IQ
Deep Dive (@aycabas). Creates a single-tenant Entra app with the delegated
``WorkIQAgent.Ask`` scope, a Foundry ``RemoteA2A`` OAuth2 connection targeting the Work IQ
A2A gateway, and a ``work-iq-tools`` toolbox exposing Work IQ to the hosted ``workmate-agent``.
"""
...
async def get_or_create_application(
    ...
    application = await graph_client.applications.post(
        Application(
            display_name=os.getenv("WORK_IQ_ENTRA_APP_NAME", "Work IQ Deep Dive Agent"),
...
def create_toolbox(endpoint: str, toolbox_name: str, connection_name: str) -> None:
    ...
    version = project.toolboxes.create_version(
        ...
        description="Microsoft 365 work context tools for the Workmate agent.",
    )
